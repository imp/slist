	...
	if (capacity != 0xFFFFFFFF) {
		...
		if (cdb[8] & SBC_CAPACITY_PMI) {
			lba = cdb[2] << 24 | cdb[3] << 16 |
			    cdb[4] << 8 | cdb[5];
			if (lba >= capacity)
				cap->capacity = BE_32(0xffffffff);
			else
				cap->capacity = (capacity - 1);
		} else {
			cap->capacity = BE_32(capacity - 1);
		}
	} else {
		cap->capacity = BE_32(capacity);
	}
	cap->lbasize = BE_32(d->d_bytes_sect);
...
sbc_read_capacity16(t10_cmd_t *cmd, uint8_t *cdb, size_t cdb_len)
{
	...
	if (cdb[14] & SBC_CAPACITY_PMI) {
		if (lba >= capacity)
			cap16->sc_capacity = BE_64(0xffffffffffffffffULL);
		else
			cap16->sc_capacity = BE_64(capacity - 1);
	} else {
		cap16->sc_capacity	= BE_64(capacity - 1);
	}
	cap16->sc_lbasize	= BE_32(d->d_bytes_sect);
...
sense_page3(disk_params_t *d, char *buf, uint8_t pc)
{
	...
	case SPC_PC_SAVED_VALUES:
		mode_fmt.data_bytes_sect	= BE_16(d->d_bytes_sect);
		mode_fmt.sect_track		= BE_16(d->d_spt);
		mode_fmt.interleave		= BE_16(d->d_interleave);
		break;
	}
